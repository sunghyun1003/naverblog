...
9. 수정이 필요하면 반려 사유를 입력한다. 기존 버전이 보존되고 자동 재작성 후 다시 `검토 필요`로 돌아오는지 확인
10. `수치와 출처`, `광고성 표현`을 확인한 뒤 승인
11. **발행 일정** → 날짜 저장
12. `원고 복사`로 NAVER 블로그 편집기에 붙여넣고 발행
13. 발행된 `https://blog.naver.com/...` 주소를 입력해 발행 완료 처리
...
| 검수·승인·반려 | 대시보드에서 처리, 반려 시 기존 버전을 보존하고 즉시 자동 재작성 |
