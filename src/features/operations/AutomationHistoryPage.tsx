        ...
        <div className="history-table" role="table" aria-label="자동화 실행 이력">
          ...
          {pageItems.map((item) => (
            <article className="history-table__row" role="row" key={item.id}>
              ...
              <div data-label="결과"><span className={`history-status history-status--${item.status}`}>{statusLabels[item.status]}</span>{formatFailedStage(item) ? <small>{formatFailedStage(item)}</small> : null}{item.draftSaved ? <small className="history-recovery-note">원고 저장됨 · 저장 원고 재시도 가능</small> : null}</div>
