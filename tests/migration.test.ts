...
test("한국어 자연스러움 결과를 저장할 수 있도록 품질 분류 제약을 확장한다", async () => {
  const sql = await readFile(new URL("../server/db/migrations/003_native_korean_quality.sql", import.meta.url), "utf8");
  assert.match(sql, /DROP CONSTRAINT IF EXISTS qa_results_category_check/);
